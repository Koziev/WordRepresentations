...
Сетка обучается как автоэнкодер на словах с дополнительным target'ом, чтобы
скрытый слой содержал информацию о битовом векторе морфологических признаках.
...
class AutoEncoder(object):
                ...
                if required_remainder is None or (iword%2)==required_remainder:
                    ...
                    if batch_index == batch_size:
                        ...
                        if mode==2:
                            ...
                            yield ({'input_layer': X_batch}, { 'word': y_word_batch, 'tags': y_tags_batch })
    ...
    def fit(self, vector_size, grammar_dict, data_folder):
        ...
        model = Model(inputs=input, outputs=[decoder_word, decoder_tags])
        model.compile(loss={'word':'categorical_crossentropy', 'tags':'mse'}, optimizer='rmsprop', metrics=['accuracy'])
        ...
        model.fit_generator(generator=self.generate_rows(grammar_dict, batch_size, mode=0),
